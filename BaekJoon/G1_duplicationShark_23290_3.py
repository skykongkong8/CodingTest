...
1. MAP과 같은 자료구조를 다차원으로 만들다보면 여러가지 문제가 발생한다.
    - 메모리 참조 문제 -> 형성하는 것 까지는 문제 없을지 몰라도 (사실 문제 있음) 재참조하거나 수정하는 과정에서 문제가 많이 발생한다.
    - 시간 복잡도 문제 -> 찾거나, 삭제하거나, 수정하거나 등의 시도를 할 때, 괜히 remove, sort 따위의 함수를 쓰다 보니 
    시간복잡도가 exponentially increasing하는 경우가 발생한다.

    -> 단순하게 가져가자.

2. pop() 메소드를 까먹고 있었다. 삭제함과 동시에 가져올 수 있으면서도, 인덱싱을 하지 않기 때문에 '이동'에 특화된 친구.
    -> 단 이러한 메소드를 사용할 때, deque가 매우 유용함을 인지하자.
    왜냐하면, 어짜피 pop한것을 다시 넣을 때, list라면 append를 해야하는데 그러면 pop한걸 또 pop해야 하는 상황이 오게 된다.
    그러나 만약 deque라면 appendleft를 할 수 있고, 이가 훨씬 시간 효율적이다.

3. 역시, 어짜피 디버깅해야하고, 또 특정 테스트 케이스에 대한 오류 여부를 잘 찾기 위해서는 systematic하게 print문을 작성하는 것이 중요하다.
    이를 고려하여 구조를 생각해보도록 하자.
"""