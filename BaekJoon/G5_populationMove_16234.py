...
1. connected components 찾기 (= 인덱싱을 돌면서 하는 DFS) 또나왔네
    다른점 : DFS긴한데, 모든 노드를 한번씩 방문하면서 하는게 아니고, 확실히 connected component에 들어왔을 때만 방문표시를 해주고 그러지
    않으면 재방문을 해줘야 하기 때문에 방문 표시 Flag놓는 위치를 잘~ 생각해서 놓아야함. 그래도 발상 자체가 무리스럽지는 않다.
